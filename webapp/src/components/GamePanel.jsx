...
const GamePanel = () => {
  ...
  const resetGame = () => {setCorrectCount(0);setIncorrectCount(0);setCurrentQuestionIndex(0);startGame(); setGameEnded(false);setSelectedAnswer(null);setQuestions([]);setQuestionData({ question: '', image: '', options: [], correctAnswer: '' });setInitialLoading(true);getQuestions();};
...
useEffect(() => {startGame();}, []);
  ...
  if (gameEnded) {endGame();const performanceMessage = correctCount >= TOTAL_QUESTIONS / 2 ? "¡Buen trabajo!" : "¡Sigue intentando!";
    return (<ThemeProvider theme={theme}><Grid container style={{height: '100vh',overflow: 'hidden',justifyContent: 'center',alignItems: 'center',backgroundColor: theme.palette.background.default,}}>
        ...
        <Typography variant="h6" style={{ fontSize: '0.9rem' }}>Puntuacion: {scoreRef.current.getTotalScore()}</Typography>{correctCount < 4 ? (<img src={img0_4} alt="Nivel 0-4" style={{ width: '100%', maxWidth: '400px' }} />) : correctCount >= 4 && correctCount < 7 ? (<img src={img4_7} alt="Nivel 4-7" style={{ width: '100%', maxWidth: '400px' }} />) : ( <img src={img7_10} alt="Nivel 7-10" style={{ width: '100%', maxWidth: '400px' }} />)}
